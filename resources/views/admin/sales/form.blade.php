<div class="row">
    ...
    <div class="col-md-4 form-group{{ $errors->has('warehouse_id') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-12">
            ...
            <select class="form-control" name="warehouse_id" id="warehouse_id" v-model="warehouse_id" required>
...
<div class="row">
    <div class="col-sm-12 px-3">
        <input type="text" class="form-control p-4" name="add_item" id="add_item" :data-warehouse="warehouse_id" data-inStock="1" placeholder="Please add products to order list">
    </div>
</div>
                ...
                <table id="poTable" class="table table-striped table-bordered table-condensed table-hover">
                    ...
                    <tr v-for="(item, index) in products">
                        <td>
                            ...
                            <input v-if="item.sale_item_id" :name="`sale_items[${index}]['sale_item_id']`" type="hidden" :value="item.sale_item_id">
                            <input :name="`sale_items[${index}][product_id]`" type="hidden" :value="item.product_id">
                        </td>
                        <td class="text-right">
                            <span class="text-right">@{{ item.price }}</span>
                        </td>
                        <td class="text-center">
                            <input v-model="item.quantity" @input="handleChangeQty" class="form-control text-center" :name="`sale_items[${index}][quantity]`" type="number" min="1" value="1">
                        </td>
                        <td class="text-right">
